...
- **`buildcage-runc`**, a wrapper around BuildKit's own `buildkit-runc`, makes each step trust the
  ...
  a copy it cannot take out fails the build rather than reaching the image. That includes a copy
  inside a binary other than a JVM keystore, since cutting it out would shift the rest of the file. Reading the layer back
