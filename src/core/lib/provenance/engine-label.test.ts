...
describe("checkImageEngine", () => {
  it("accepts a label whose suffix names the requested engine", () => {
    ...
    expect(() => check("3.1.0-universal", "universal")).not.toThrow();
  });

  it("accepts a prerelease version carrying the engine suffix", () => {
    expect(() => check("3.1.0-rc1-universal", "universal")).not.toThrow();
    expect(() => check("3.1.0-rc1-inspect", "inspect")).not.toThrow();
  });

  it("accepts any prerelease the release workflow can tag", () => {
    expect(() => check("3.2.0-beta.1-inspect", "inspect")).not.toThrow();
    expect(() => check("3.2.0-alpha-universal", "universal")).not.toThrow();
    expect(() => check("3.2.0-rc.2.1-inspect", "inspect")).not.toThrow();
  });
  ...
  it("rejects a prerelease the release workflow cannot tag", () => {
    ...
    expectRejected("3.2.0-x-y-inspect", "inspect");
    expectRejected("3.2.0-beta..1-inspect", "inspect");
    expectRejected("3.2.0--inspect", "inspect");
  });

  it("ignores the version half, which a floating ref or SHA pin does not match", () => {
    expect(() => check("3.0.1-inspect", "inspect")).not.toThrow();
  });

  it("rejects the universal image served for an inspect tag", () => {
    const err = expectRejected("3.1.0-universal", "inspect");
  ...
  it("rejects an unsuffixed release version, which names no engine", () => {
    expectRejected("3.1.0", "inspect");
    expectRejected("3.1.0", "universal");
  });
  ...
  it("rejects an engine image served for a universal tag", () => {
    ...
    expectRejected("3.2.0-beta.1-inspect", "universal");
  });

  it("rejects a suffix the action does not offer", () => {
    expectRejected("3.1.0-future", "universal");
    expectRejected("3.1.0-rc1-future", "universal");
  });
