...
function reverseZoneLines(proxyAddress, ttlSeconds) {
	return [
		...
		"    view reverse {",
		"      expr name() matches '^(([0-9]{1,3}[.]){1,4}in-addr[.]arpa|([0-9a-fA-F][.]){1,32}ip6[.]arpa)[.]$'",
		"    }",
		"    template IN PTR {",
		"      rcode NXDOMAIN",
		`      authority "${`{{ .Zone }} ${ttlSeconds} IN SOA ns.buildcage.invalid. hostmaster.buildcage.invalid. 1 ${ttlSeconds} ${ttlSeconds} ${ttlSeconds} ${ttlSeconds}`}"`,
		"    }",
...
const HOST_LITERAL_ILLEGAL = /\\[[\]]/, RE2_UNSUPPORTED = /^(?:\(\?<?[=!]|\\[1-9]|\\k<)/;
...
function compileSchemeRules(hostRules, urlRules, scheme) {
	...
	for (let rule of urlRules ?? []) if (rule.scheme === scheme) {
		if (rule.isRegex) {
			out.push({
				id: "",
...
function detectFrontend(spec) {
	let { listenPort, tlsStagePort, plainStagePort, ipRules, tlsHosts, hasResolver } = spec, hasPassthrough = ipRules.length > 0 || tlsHosts.length > 0, l = [];
	if (l.push("# One listener for everything redirected here. The first bytes say whether", "# this is a handshake or a plain request, so no port has to be declared as", "# one or the other in advance.", "frontend detect", `    bind *:${listenPort}`, "    mode tcp", "    tcp-request inspect-delay 5s", ""), hasPassthrough) {
		l.push("    # Captured now, since the request buffer is gone by log time.", "    tcp-request content set-var(txn.sni) req.ssl_sni,regsub([^A-Za-z0-9._-],_,g)"), ipRules.some((rule) => rule.hostMatch === "hostPort") && l.push("    tcp-request content set-var-fmt(txn.dst_str) %[dst]:%[dst_port]"), tlsHosts.some((host) => host.hostMatch === "hostPort") && l.push("    tcp-request content set-var-fmt(txn.sni_port) %[req.ssl_sni]:%[dst_port]"), l.push("", "    # Passed through untouched: judged before anything is decrypted.");
		...
		let conds = [...ipRules.map((r) => `${r.id}_dst${r.port ? ` ${r.id}_port` : ""}`), ...tlsHosts.map((h) => `${h.id}_sni${h.port ? ` ${h.id}_port` : ""}`)];
		if (l.push("", ...conds.map((cond) => `    tcp-request content set-var(txn.pass) int(1) if ${cond}`), "    tcp-request content set-var(txn.proto) str(tls) if { req.ssl_hello_type 1 }", "    tcp-request content set-var(txn.proto) str(tcp) unless { req.ssl_hello_type 1 }"), tlsHosts.length > 0 && hasResolver) {
			l.push("");
...
function ruleBlock(rules, mode, scheme) {
	...
	rules.some((r) => r.hostMatch === "hostPort") && lines.push(`    http-request set-var-fmt(txn.host_port) %[hdr(host),${HOST_ONLY}]:%[dst_port]`), rules.some((r) => r.hostMatch === "hostBareFull") && lines.push(`    acl is_default_port dst_port ${DEFAULT_PORT[scheme]}`, `    http-request set-var(txn.host_bare) hdr(host),${HOST_ONLY}`, `    http-request set-var-fmt(txn.host_full) %[hdr(host),${HOST_ONLY}]:%[dst_port]`), rules.some((r) => r.hostMatch === "wildcard") && lines.push(`    http-request set-var(txn.host) hdr(host),lower,${HOST_ONLY}`);
	...
	for (let rule of rules) {
		...
		else if (rule.hostMatch === "hostBareFull") lines.push(`    http-request set-var(txn.${rule.id}_ok) bool(false)`, `    http-request set-var(txn.${rule.id}_ok) bool(true) if is_default_port { var(txn.host_bare) -m reg -i ${hostRegex} }`, `    http-request set-var(txn.${rule.id}_ok) bool(true) if { var(txn.host_full) -m reg -i ${hostRegex} }`, `    acl ${rule.id}_host var(txn.${rule.id}_ok) -m bool`);
		else {
